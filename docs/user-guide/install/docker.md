...
assignees:
- ashvayka
title: Installing Klyff using Docker (Linux or Mac OS)
description: Installing Klyff IoT Platform using Docker (Linux or Mac OS)
...
This guide will help you to install and start Klyff using Docker on Linux or Mac OS.
...
In this instruction [thingsboard/tb-postgres](https://hub.docker.com/r/thingsboard/tb-postgres/) image will be used. It contains a single instance of Klyff with PostgreSQL database.
...
- `~/.mytb-data:/data`   - mounts the host's dir `~/.mytb-data` to Klyff DataBase data directory
- `~/.mytb-logs:/var/log/thingsboard`   - mounts the host's dir `~/.mytb-logs` to Klyff logs directory
