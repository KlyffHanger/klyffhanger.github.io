...
description: Klyff IoT platform cluster setup with Kubernetes and OpenShift guide
...
This guide will help you to setup Klyff in cluster mode using OpenShift. 

## Prerequisites

Klyff Microservices run on the Kubernetes cluster. To deploy OpenShift cluster locally you'll need to have Docker CE to run OpenShift containers and OpenShift Origin itself. 
...
## Step 2. Clone Klyff CE Kubernetes scripts repository
...
To see how to reach your Klyff application on cluster, login as ***developer*** user (default password is ***developer*** too), open `thingsboard` project, then go to `Application -> Routes` menu and you'll see all your configured routes.
The *root* route should look like `https://tb-route-node-root-thingsboard.127.0.0.1.nip.io/`.

When you open it, you should see Klyff login page.
