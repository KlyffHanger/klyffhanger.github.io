...
title: What is Klyff IoT Gateway?
description: Features and advantages of Klyff IoT Gateway

---

The Klyff **IoT Gateway** is an open-source solution that allows you to integrate devices connected to legacy and third-party systems with Klyff.  

Klyff is an open-source IoT platform for data collection, processing, visualization, and device management. See [**What is Klyff?**](/docs/getting-started-guides/what-is-thingsboard/) if you are new platform user.

<object width="95%" data="/images/gateway/python-gateway-animd-ff.svg"></object>

#### Gateway features

The Klyff IoT Gateway provides the following features:
 ...
 - **Automatic reconnect** to the Klyff cluster.
...
The main components of Klyff IoT Gateway are listed below.
...
Converters are responsible for converting data from protocol specific format to/from Klyff format.
Converters are invoked by Connectors. Converters are often specific to protocol supported by Connector.
There are uplink and downlink converters. The uplink converter is used to convert data from specific protocol to Klyff format.
The downlink converter is used to convert messages from Klyff to specific protocol format.
...
The Event Storage is used to temporary store the telemetry and other events produced by Connectors until they are delivered to Klyff.
...
The Gateway communicates to Klyff via MQTT protocol and uses API described [here](/docs/reference/gateway-mqtt-api/).
Klyff Client is a separate thread that polls Event Storage and delivers messages once connection to Klyff is active.  
Klyff Client supports monitoring of the connectivity, batching the events for performance improvement and many other features.

**Gateway Service**

The Gateway Service is responsible for bootstrapping the Connectors, Event Storage and Klyff Client. 
This Service collects and periodically reports statistics to Klyff about incoming messages and connected devices.
