...
description: ODBC support for Klyff IoT Gateway
...
This guide will help you get familiar with ODBC connector configuration for Klyff IoT Gateway.
...
2. Install ODBC driver(s) for database(s) the Klyff gateway needs to connect to.
...
1. Valid SQL *SELECT* statement that meets requirements of SQL dialect of the database the Klyff gateway needs to connect to.
...
These **optional** subsections provide information on which database columns are to be treated as attributes, which ones as time series keys, and what pre-processing tasks should be done before sending data to the Klyff server.
