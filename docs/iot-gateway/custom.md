...
1. "name" - name of the device for Klyff instance.  
...
7. "attributesUpdates" - objects array with a configuration for processing attribute update request from Klyff.
...
class CustomSerialConnector(Thread, Connector):    # Define a connector class, it should inherit from "Connector" class.
    ...
    def on_attributes_update(self, content):    # Function used for processing attribute update requests from Klyff
        log.debug(content)
            ...
            if device_config is not None:
                ...
                if device_config.get("attributeUpdates") is not None:
                    ...
                    for request in requests:
                        attribute = request.get("attributeOnKlyff")
                        log.debug(attribute)
...
The purpose of the converter is to convert data from devices to the Klyff format.
...
class CustomSerialUplinkConverter(Converter):    # Definition of class.
    ...
    def convert(self, config, data: bytes):    # Method for conversion data from device format to Klyff format.
...
This dictionary will be converted into json and gateway will send it to Klyff instance.
...
### Step 6. Check a result in the Klyff instance

To check the result, you should connect device, and go to the Klyff UI into "Devices" tab.  
...
**on_attributes_update** -- gateway call it when receives AttributeUpdates request from Klyff server to device with this connector.  
**server_side_rpc** -- gateway call it when receives ServerRpcRequest from Klyff server.  
...
Method is being called when gateway receive AttributeUpdates request from Klyff.  

```python
def on_attributes_update_method(self, content):
```
*self* -- current object.  
*content* -- dictionary with data from Klyff server.  
...
*req_id* - Id of RPC request from Klyff  
...
```
 ...
 - If with response in content should be any dictionary with content that you want send to Klyff as response.
...
**convert** -- Method for conversion data from device format to Klyff data format.  
...
In the example used to save the converter configuration and create a template for the result's dictionary with Klyff data format

##### convert method

Method for conversion data from device format to Klyff data format.
...
Gateway will convert this data into json and send it to the Klyff instance.
