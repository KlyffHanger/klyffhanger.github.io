...
description: Installation structure and configuration of Klyff IoT Gateway 
...
/var/log/thingsboard-gateway                      - Logs folder.
    ...
    tb_connection.log                             - Logs for connection to the Klyff instance.
```
...
version: '3.4'
services:
  # Klyff IoT Gateway Service Configuration
  tb-gateway:
...
The following environmental variables can be used to configure the Klyff IoT Gateway:

| **ENV variable**     | **Default value**             | **Description**                                                                   |
|----------------------|-------------------------------|-----------------------------------------------------------------------------------|
| TB_GW_HOST           | **host.docker.internal**      | Hostname or IP address of Klyff server.                                     |
| TB_GW_PORT           | **1883**                      | Port of MQTT service on Klyff server.                                       |
| TB_GW_ACCESS_TOKEN   | **YOUR_ACCESS_TOKEN**         | Access token for the gateway from Klyff server.                             |
| TB_GW_CA_CERT        |                               | Path to CA certificate file.                                                      |
| TB_GW_PRIVATE_KEY    |                               | Path to private key file.                                                         |
| TB_GW_CERT           |                               | Path to certificate file.                                                         |
| TB_GW_CLIENT_ID      |                               | MQTT client id for the gateway form Klyff server.                           |
| TB_GW_USERNAME       |                               | MQTT username for the gateway form Klyff server.                            |
| TB_GW_PASSWORD       |                               | MQTT password for the gateway form Klyff server.                            |
| TB_GW_RATE_LIMITS    | **15:1,300:60,**              | Messages rate limit to Klyff server in `MESSSAGE_COUNT:TIME,` format.       |
| TB_GW_DP_RATE_LIMITS | **15:1,300:60,**              | Data points rate limit to Klyff server in `DATA_POINTS_COUNT:TIME,` format. |
| TB_GW_LOGS_PATH      | **/thingsboard_gateway/logs** | Path to the logs folder.                                                          |

## General configuration file

The main configuration file that is used for connection to Klyff platform instance and enable/disable connectors. 

The example configuration file provided below sets a connection to the Klyff instance located at 
...
+ **thingsboard** -- Configuration for connecting to Klyff platform.
...
| host                     | **thingsboard.cloud**                        | Hostname or IP address of Klyff server.                                           |
| port                     | **1883**                                     | Port of MQTT service on Klyff server.                                             |
...
This subsection is used to configure collecting statistics data and sending them to Klyff Gateway device attributes.
...
This subsection is optional and used to filter allowed devices to send data to Klyff.
...
Configs in storage subsection provides configuration for saving incoming data before it will be send to Klyff platform.
