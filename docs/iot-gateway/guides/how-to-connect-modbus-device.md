...
assignees:
- zbeacon
title: How to connect Modbus device to Klyff using the Klyff IoT Gateway
description: Understand how to connect Modbus device to Klyff using the Klyff IoT Gateway 
...
We want to write **temperature** (register address is 0) and **humidity** (register address is 1) as the telemetry to Klyff and **batteryLevel** (register address is 2) as the device client-side attribute.      
...
2. General device configuration. In this section we have defined main settings of our Modbus device (e. g. device name within Klyff — TH_sensor, unit id — 1 etc.). You can read more about available parameters here.
...
4. Attributes configuration. In this section we have defined the settings for batteryLevel attribute within Klyff. You can read more about available parameters here.
...
Check data in your Klyff instance.  
