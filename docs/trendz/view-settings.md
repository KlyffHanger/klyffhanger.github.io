...
Let's assume that we have 2 buildings and 2 sensors registered in the Klyff and relations configured among them:
...
The ability to get filtering parameters for the Klyff dashboard using the URL line (query parameters) of the browser.

##### SET filter values to url
The ability to set filtering parameters for the Klyff dashboard using the URL line (query parameters) of the browser.

##### Hidden filters
Hide specific filters when displayed in the Klyff dashboard

##### Row-click entity
An entity that will be passed on a click on a view element (table row, bar of chart, etc.) to the Klyff alias. This is usually used to change filters of view in other widgets.
...
##### Cache report
If the query plan is not changed will be return cached View Report without loading data from Klyff.
[**More details you can find here**](/docs/trendz/cache-settings/)

##### Enable caching
In cases when data can be grouped by fixed intervals, like Hour or Date, will be load already computed\aggregated field value from cache instead of reloading data from Klyff.
