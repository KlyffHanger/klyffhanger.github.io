...
assignees:
- vparomskiy
title: Installing Klyff Trendz Analytics  on CentOS/RHEL
description: Installing Klyff Trendz Analytics  on CentOS/RHEL
...
In small and medium installations Trendz can be installed **on the same** server with Klyff.
...
### Step 4. Configure connection with Klyff Platform
...
Add Klyff REST API URL that would be used for communicating with Klyff Platform. In most cases, when Trendz installed
in the same server with Klyff, API_URL would be **http://localhost:8080**. Otherwise you should use Klyff domain name.

```bash
# Klyff URL that will be used by Trendz
...
**Trendz and Klyff hosted on same server**

Use this section if HAProxy/Let’s Encrypt already installed in the server and HTTPS enabled for Klyff.
...
### Step 10. Host Klyff and Trendz on the same domain
Klyff and Trendz can share same domain name. In this case Klyff web page would be loaded using following link:
