...
building_energy_dashboard:
  0:
    ...
    title: 'Create new dashboard in Klyff'
  1:
...
* Create Klyff dashboard to visualize data with filtering options

## Getting started:

### Prerequisites
Energy meters already installed in the building and connected to Klyff via MQTT API. To simplify data aggregation and analysis, each meter has 2 attributes:
...
### Step 6: Create energy consumption analytic dashboard in Klyff
...
consumption sources and areas. We begin with adding all Trendz views that we created on one dashboard in Klyff:

* In Klyff create dashboard with name **Energy consumption report**
...
* Return to Klyff **Energy consumption report** dashboard and tune dashboard layout.
...
* Add Klyff table widget `Cards` -> `Entities table` to the dashboard. It will show all buildings that are visible for the user.
