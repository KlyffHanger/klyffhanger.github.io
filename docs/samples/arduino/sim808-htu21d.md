...
description: Klyff IoT Platform sample for humidity and temperature data upload over HTTP using Arduino UNO, SIM808 GSM shield and HTU21D sensor.
...
Collected data is pushed via HTTP to Klyff server for storage and visualization.
...
Arduino UNO pushes data to Klyff server via HTTP protocol by using [Arduino Klyff SDK](https://github.com/thingsboard/Klyff-Arduino-MQTT-SDK).
...
### Step 2. Install Arduino Klyff SDK and dependencies
...
1. Find and install **Klyff Arduino SDK** and **PubSubClient by Nick O'Leary** libraries.
...
- `THINGSBOARD_SERVER` - Klyff HOST/IP address that is accessible from within your wifi network. Specify "demo.thingsboard.io" if you are using [live demo](https://demo.thingsboard.io/) server.
...
After application will be uploaded and started it will try to connect to Klyff node using HTTP and upload "humidity" and "temperature" timeseries data once per second.
...
Finally, open Klyff Web UI. You can access this dashboard by logging in as a tenant administrator. Use

 - login: tenant@thingsboard.org
 - password: tenant

in case of local Klyff installation.
