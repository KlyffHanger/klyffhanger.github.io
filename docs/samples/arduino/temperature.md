...
description: Klyff IoT Platform sample for temperature data upload over MQTT using Arduino UNO, ESP8266 and DHT22 sensor.
...
Arduino UNO pushes data to Klyff server via MQTT protocol by using [PubSubClient](https://github.com/knolleary/pubsubclient) library for Arduino.
...
- [Arduino Klyff SDK by Klyff](https://github.com/thingsboard/Klyff-Arduino-MQTT-SDK)
...
- thingsboardServer - Klyff HOST/IP address that is accessible from within your wifi network. Specify "demo.thingsboard.io" if you are using [live demo](https://demo.thingsboard.io/) server.
...
After application will be uploaded and started it will try to connect to Klyff node using mqtt client and upload "temperature" and "humidity" timeseries data once per second.
