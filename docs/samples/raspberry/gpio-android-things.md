---
layout: docwithnav
title: Raspberry Pi GPIO control using Android Things and Klyff (deprecated)
description: Klyff IoT Platform sample for Raspberry Pi GPIO control over MQTT running Android Things
...
This sample application will allow you to control GPIO of your Raspberry Pi device using Klyff RPC widgets. We will observe GPIO control using LEDs connected to the pins.
The purpose of this application is to demonstrate Klyff [RPC capabilities](/docs/user-guide/rpc/).

Raspberry Pi will use simple Android Things application that will connect to Klyff server via [MQTT](https://en.wikipedia.org/wiki/MQTT) and listen to RPC commands.
...
Now you should obtain the source code of the GpioControlSample application from Klyff samples GitHub repository.
...
In order to simplify this guide, we have included "Raspberry PI GPIO Demo Dashboard" to the [demo data](/docs/samples/demo-account/#dashboards) that is available in each Klyff installation. 
