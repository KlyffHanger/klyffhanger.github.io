...
This guide contains step-by-step instruction how to to connect your SODAQ NB-IoT boards to Klyff Professional Edition (PE) through the T-Mobile NB IoT network.
...
We expect you have a very basic knowledge about Klyff. Otherwise we do recommend to complete the following guides:
...
We will use "UDP" platform integration to consume data from T-Mobile NB IoT Network and automatically register devices in Klyff.
Besides configuring the integration, we will also setup Klyff to decode incoming data, store it in the database, visualize on the dashboard and generate alarms based on configurable thresholds.
 ...
 * Klyff will automatically create device with type "tracker" and name equal to IMEI;
...
Also, let’s make sure Klyff is properly configured using simple **echo** command and **netcat** utility.
