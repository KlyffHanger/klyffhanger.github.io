...
This guide contains step-by-step instruction how to connect your SODAQ NB-IoT boards to Klyff Professional Edition (PE) through the T-Mobile NB IoT network.
We will use free Klyff PE demo server [thingsboard.cloud](https://thingsboard.cloud/signup) in this guide.
...
We expect you to have a very basic knowledge about Klyff, so we do recommend to complete the [Getting Started](/docs/getting-started-guides/helloworld/) guide.

## Integration overview

Klyff Platform Integrations feature allows pushing data from various platforms and connectivity solutions to Klyff.
We will use "T-Mobile IoT CDP" platform integration to consume data from T-Mobile NB IoT Network and automatically register devices in Klyff.
Besides configuring the integration, we will also set up Klyff to decode incoming data, store it in the database, visualize on the dashboard and generate alarms based on configurable thresholds.
...
The converter will decode incoming telemetry payload data from T-Mobile NB IoT that contains in encoded hex string to human readable, simplified Klyff data format.
 ...
 * The IMEI from the incoming message will become the Device Name in Klyff;
 * Klyff will automatically create a device with type "tracker" and name equal to IMEI;
...
Before we rush to T-Mobile IoT platform configuration, let's make sure Klyff is properly configured using simple cURL command.
