...
description: Klyff IoT Platform sample for temperature data upload over MQTT using NodeMCU and DHT11 sensor.
...
The DHT11 sensor is connected to [NodeMCU](https://en.wikipedia.org/wiki/NodeMCU). NodeMCU push data to Klyff server via [MQTT](https://en.wikipedia.org/wiki/MQTT) protocol.
...
Our application consists of three *.lua* files:
 ...
 - config.lua - configuration file, where we define a custom configuration. 
   You need to modify this file in order to setup your wifi network parameters and address of Klyff server.
...
In order to simplify this guide, we have included "Temperature & Humidity Demo Dashboard" to the [demo data](/docs/samples/demo-account/) that is available in each Klyff installation.
