---
layout: docwithnav
title: ESP32 OTA using Klyff
description: Klyff IoT Platform sample for ESP32 OTA update using Klyff
hidetoc: "true"
---
...
This sample application allow you to deliver a new firmware images to EPS32 with using Klyff and OTA.
...
- Account in Klyff application. You can use your own instance or [Klyff Cloud](https://thingsboard.cloud)
...
Download, import [Check is ESP32 firmware synced](/docs/samples/esp32/resources/check_is_esp32_firmware_synced.json) rule chain into Klyff and save it:
...
4. Download and import [OTA widgets (Klyff v3.x)](/docs/samples/esp32/resources/ota_widgets_v2.json) or [OTA widgets (Klyff v2.x)](/docs/samples/esp32/resources/ota_widgets.json) widgets group to Widgets Library to allow specifying of firmware URL and version and send OTA configuration to ESP32.
5. Download and import [OTA for ESP32 (Klyff v3.x)](/docs/samples/esp32/resources/ota_for_esp32_v2.json) or [OTA for ESP32 (Klyff v2.x)](/docs/samples/esp32/resources/ota_for_esp32.json) dashboard to Dashboards Group. The dashboard has the alias *ESP32_OTA_alias* for the devices with type *ESP32_OTA*.
...
1. Clone the sample's sources from [ESP32 OTA](https://github.com/thingsboard/esp32-ota) Klyff repository.
...
Open the terminal and execute next command:
    ...
    make menuconfig
    ```
    In the opened menu select *Klyff OTA configuration* sub-menu and enter the valid parameters to the fields:
    -  *WiFi SSID* - login of your WiFi access point
    -  *WiFi Password* - password of your WiFi access point
    -  *MQTT broker URL* - Klyff MQTT endpoint
    -  *MQTT broker port* - Klyff MQTT port
    -  *MQTT access token* - device's access token in Klyff
...
Go to the [root sample directory](https://github.com/thingsboard/esp32-ota) and execute the next command
    ...
    make flash monitor
    ```
    If compilation and flashing were successful and ESP32 connected to Klyff, the next log messages are printed:
    ...
    4. After the connection to Wi-Fi access point was established, the application checks does the flash memory contain MQTT URL, port and Klyff access token.
    ...
    5. The application is trying to connect to Klyff over MQTT.
    6. After the connection was established, the application fetching shared attributes *targetFwUrl* and *targetFwVer* from Klyff.
    It allows to cover the case when EPS32 was powered off or lose the connection with Klyff but the shared attributes were update that time.
    ...
    Because any shared attributes were not created in Klyff yet (they will be created through the widget later), for now OTA procedure is skipped.
    7. The application started to execute a custom task (for example send some telemetry or attributes data to Klyff) and waits for the update of the shared attributes *targetFwUrl* and *targetFwVer* to perform OTA.

## Performing OTA from Klyff
...
After the firmware version and URL were updated on the *OTA control* widget, Klyff sends a MQTT message with the shared attributes to *v1/devices/me/attributes* MQTT topic.
...
If the value of *FIRMWARE_VERSION* defined in [main.h](https://github.com/thingsboard/esp32-ota/blob/master/main/main.h) isn't equal to the firmware version received from Klyff, OTA update process will started and *monitor* utility outputs the next logs:
...
The application's flow is the next:
  ...
  - during OTA process all other tasks could be executed with some delays.
    For example, the application sends a telemetry data (in this sample it is the current value of *counter* variable) to Klyff with not determined period.
...
Let's review the logs differences in comparison when the firmware was flashed to *factory* partition:
  ...
  - MQTT access token for Klyff also was loaded from the flash memory
