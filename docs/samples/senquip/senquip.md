---
layout: docwithnav
title: Connecting a Senquip telemetry device to the Klyff
description: Configuration of a Senquip telemetry device to enable it to send telemetry data to the Klyff.
...
A similar guide is also available as an [application note](https://docs.senquip.com/app_notes/APN0015%20Rev%201.0%20-%20Connecting%20Senquip%20Devices%20to%20the%20Klyff.pdf) from the Senquip website.
...
### Configuring the Klyff endpoint

Senquip devices can send to the Senquip Portal and a second endpoint at the same time.  In this example, the Klyff will be confirgured as the second endpoint, using MQTT as the transport.
...
* [Step 5] Select a username to be used on the Klyff, we have chosen "Senquip" (Setup/Endpoint/MQTT/Username = "Senquip").
...
It will be assumed that the user has an account on the [Klyff Cloud](https://thingsboard.cloud/).  All device configuration described in the steps below will be performed using the [Klyff Cloud](https://thingsboard.cloud/).
