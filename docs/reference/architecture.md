...
description: Klyff IoT Platform architecture
...
Klyff is designed to distribute workload across multiple nodes without a single point of failure.
Each Klyff node is identical and can handle request from both device and server-side applications.
...
Klyff supports [**MQTT**](/docs/reference/mqtt-api/), [**LwM2M**](/docs/reference/lwm2m-api/) , [**CoAP**](/docs/reference/coap-api/) and [**HTTP**](/docs/reference/http-api/) protocols for device connectivity.
It is possible to plugin support of different protocols or customize existing implementations.

#### Rule Engine

Klyff [Rule Engine](/docs/user-guide/rule-engine/) allows to process messages from devices and trigger configurable processing modules called Plugins.

#### Core Services

Klyff contains set of core services that allow managing the following entities:
...
Every Klyff server provides REST API for registered users. 
...
Learn more about Klyff REST APIs [here](/docs/reference/rest-api/)
...
Klyff uses own Actor System implementation (sharpened for our use cases) with following actor hierarchies.
...
Klyff uses Zookeeper for service discovery.
All Klyff nodes are identical and registered as ephemeral in Zookeeper. Apache Curator [path cache receipt](http://curator.apache.org/curator-recipes/path-cache.html) is used to keep track of all available sibling nodes.

###### Consistent Hashing

Klyff adopts [consistent hashing](https://en.wikipedia.org/wiki/Consistent_hashing) to ensure scalability and availability.
...
The illustration below demonstrates how Klyff handles RPC request to Device D1.
...
As a system administrator, you are able to configure Klyff to use secure sockets layer for HTTP(s) and MQTT transports.
DTLS for CoAP is not supported yet.

### Device authentication

Klyff is designed to support many types of device credentials.
...
Klyff uses following main third-party projects:
