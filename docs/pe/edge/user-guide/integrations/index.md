...
Embedded integration is running in the main Klyff Edge process. 
...
Cons:
* consume resources allocated to main Klyff Edge process: network connections, OS threads and CPU cycles;
...
Those brokers and/or servers don't have dedicated external IP address, so Klyff Edge can't connect to them directly.
...
Remote integration will stream the data to the Klyff Edge deployed in the network once the network connection is available.

Pros:
* enables integration with servers deployed in the local network;
* isolates the integration process from main Klyff Edgeprocess;
