...
assignIntegration:
    ...
    3:
        ...
        title: 'Login to your <b>Klyff Edge</b> instance and open Integrations page - placeholder is going to be replaced by attribute value'
...
OPC UA Integration allows you to stream data from the OPC UA server to Klyff Edge and converts the device payloads to the Klyff Edge format.

<object width="100%" style="max-width: max-content;" data="/images/user-guide/integrations/opc-ua-integration.svg"></object>

In this tutorial, we will configure the integration between Klyff Edge and OPC-UA 
...
**Downlink Converter** parses and transforms the data sent from Klyff Edge to the format that is consumed by existing device(s).
...
The purpose of the decoder function is to parse the incoming data and metadata to a format that Klyff Edge can consume.
...
For sending Downlink messages from the Klyff Edge to the OPC UA node, we need to define a Downlink Converter.
