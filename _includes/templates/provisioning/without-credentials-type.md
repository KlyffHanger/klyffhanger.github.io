...
THINGSBOARD_HOST = "{{mqttHostName}}"  # Klyff instance host
THINGSBOARD_PORT = 1883  # Klyff instance MQTT port
...
def on_tb_connected(client, userdata, flags, rc):  # Callback for connect with received credentials
    if rc == 0:
        print("[Klyff client] Connected to Klyff with credentials: %s" % client._username)
    else:
...
To communicate with Klyff we will use asyncio and aiocoap modules, so we should install it: <br><br>
