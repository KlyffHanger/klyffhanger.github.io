...
to get familiar with Klyff. We also recommend to review the [Device Profiles](/docs/{{docsPrefix}}user-guide/device-profiles/) documentation first.
...
Klyff acts as an MQTT Server which support the SparkPlug payload and topic structure and allows connections from the 
...
Klyff decodes the device metrics from the Sparkplug payload and stores it as a corresponding device 
...
**NOTE:**
<br>
Klyff supports **Sparkplug™ B** payloads only.
{% endcapture %}
{% include templates/info-banner.md content=difference %}

## Getting started

This guide will teach us how to: connect Sparkplug EoN node to Klyff,
collect device metrics and store them as Klyff time series data,
...
You may push update to Sparkplug node/device metric from Klyff via shared attribute update or RPC command. 

#### Update Metrics using shared attributes

Klyff [Shared Attributes](/docs/{{docsPrefix}}user-guide/attributes/#shared-attributes) are used to deliver metric value updates to the device.
...
#### Update Metrics  using the Klyff RPC command from server to MQTT EON/Device

Klyff supports on-demand update to metrics of the Sparkplug EoN Node or Device using RPC(Remote Procedure Call) feature. We also use term "command" instead of RPC for simplicity.
