...
Klyff server nodes act as a CoAP Server that supports both regular and observe requests.
...
In order to publish telemetry data to Klyff server node, send POST request.
...
In order to publish client-side device attributes to Klyff server node, send POST request.
...
In order to request client-side or shared device attributes to Klyff server node, send GET request.
