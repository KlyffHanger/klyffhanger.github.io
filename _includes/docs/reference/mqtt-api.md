...
Klyff server nodes act as an MQTT Broker that supports QoS levels 0 (at most once) and 1 (at least once) and a set of [configurable](/docs/{{docsPrefix}}user-guide/device-profiles/#mqtt-device-topic-filters) topics.
...
* **0x00 Connected** - Successfully connected to Klyff MQTT server.
...
In order to publish telemetry data to Klyff server node, send PUBLISH message to the following topic:
...
In case your MQTT device is a gateway, Klyff will send a server-side RPC (notification) about changes on provisioned device entities.  
...
A typical RPC to retrieve session limits from the Klyff server looks like this:
```json
{
...
When Klyff initiates an MQTT device firmware update, it sets the fw_title, fw_version, fw_checksum, fw_checksum_algorithm shared attributes.
