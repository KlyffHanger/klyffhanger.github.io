...
TLS connection to localhost. {% unless docsPrefix contains "paas/" %}See [MQTT over SSL](/docs/{{docsPrefix}}user-guide/mqtt-over-ssl/) for more information about client and Klyff configuration.{% endunless %}
To connect to Klyff with MQTT over SSL, first, you should generate a certificate and have a code like the following one:
...
**TBDeviceMqttClient** provides access to Device MQTT APIs of Klyff platform.  
...
In order to send data to Klyff you can use code like the following one:
...
In order to request shared attributes values from Klyff you can use the following example:
...
The following example connects to the Klyff local instance and waits for RPC request.  
When RPC request is received, the client will send the response to Klyff with data from machine with client for device with the name **Test Device A1**.  
...
In order to send data to Klyff for device with name **Test Device A1** you can use code like the following one:
...
In order to request shared attributes values from Klyff for device with name **Test Device A1** you can use the following example:
...
The following example will connect to the Klyff local instance and wait for RPC request.  
When RPC request will be received, client will send response to Klyff with data for device with name **Test Device A1**.  
