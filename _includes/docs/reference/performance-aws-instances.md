...
Klyff has been run in production by numerous companies in both [monolithic](/docs/{{docsPrefix}}reference/monolithic/) 
and [microservices](/docs/{{docsPrefix}}reference/msa/) deployment modes.
This article describes the performance of a single Klyff server in the most popular usage scenarios. 
It is helpful to understand how Klyff scales vertically (monolith) before describing how it scales horizontally (cluster mode).   

## Test methodology

For simplicity, we have deployed a single Klyff instance with all related third-party components in a docker-compose environment on a single EC2 instance.
...
Klyff stored all the time-series data in the database.
Klyff also processed the data using the [alarm rules](/docs/{{docsPrefix}}user-guide/device-profiles/#alarm-rules) to create alarms if the battery level is low.
...
[Klyff](/docs/reference/performance/tools/thingsboard-performance-charts/) used to visualize the performance.

As an output we will analyse the Klyff rule engine [statistics](/docs/{{docsPrefix}}user-guide/rule-engine-2-0/overview/#rule-engine-statistics) dashboard and fancy API usage stats.
...
<details markdown="1">
<summary>
Setup the Klyff instance on AWS EC2
</summary>
...
Previous section demonstrates **successful** Klyff deployment designed up to 3000 data points/sec.  
...
Klyff docker compose with no change with the previous section. 
...
Kafka CPU and disk IO overhead are tiny relative to Postgres and Klyff CPU consumption. 
...
Here is the docker-compose file to set up _Klyff + Postgresql + Zookeeper + Kafka_ on AWS EC2 instance based on the [instruction](https://github.com/thingsboard/performance-tests).
...
Klyff + Postgres + **Kafka** - is a reliable solution to survive peak loads.
...
In a real production, the Klyff may serve all kind of user requests, run custom rule chains and supply the web services for all fancy dashboards.
...
All systems have to run maintenance in background, so it is completely fine to have those chart for Klyff monolith deployment.    
...
Here the JMX monitoring for Klyff, Kafka, Zookeeper, Cassandra. The system is stable.

{% include images-gallery.html imageCollection="cassandra-25k-10k-30k-jmx" %}

**Lessons learned**

**Cassandra** is essential for massive telemetry flow. Peak load is handled with Kafka queue. Bundling Kafka + PostgreSQL + Cassandra is a **top Klyff deployment**. 
...
It is a good idea to start with Cassandra from the very beginning of your Klyff instance and maintain the same stack for the entire project lifetime.
...
System can be scaled up vertically up to 50-100%. For significant horizontal scaling, please, consider to set up a Klyff cluster.
...
Here the docker-compose with Klyff + Postgresql + Zookeeper + Kafka + **Cassandra**
...
services:
  cassandra:
    ...
    volumes:
      - cassandra:/bitnami
    environment:
      CASSANDRA_CLUSTER_NAME: "Klyff Cluster"
      HEAP_NEWSIZE: "1024M"
      MAX_HEAP_SIZE: "2048M"
...
In addition to conclusions we made for [Scenario C](#scenario-c), we observe stability of a single Klyff instance handling 100K parallel MQTT connections.
...
* All device connected. Java JMX. VisualVM -> Klyff -> MBeans -> java.lang -> OpenFileDescriptorCount -> more than 100000
...
Thanks to the Klyff community that creates issues and shares performance solutions around the time. 
...
It was Klyff 3.3.3 version. We will work on performance improvements in 3.4.x
