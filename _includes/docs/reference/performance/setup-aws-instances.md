...
To run clear test lets spin up two instances for Klyff and for performance tool.
...
We need at least two instances: first is to run Klyff itself and second is to run the Performance test application.   

![Klyff and Performance test instances](/images/reference/performance-aws-instances/method/setup/performance_test_aws_instances.png "Klyff and Performance test instances")
...
To connect Klyff instance simply use the command below:
```bash
ssh tb
```
...
Login with ssh and run the commands both on Klyff and Performance test instances:
...
#### Step 4. Launch Klyff and third-party components using docker-compose
