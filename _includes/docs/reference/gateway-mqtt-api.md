...
The Gateway is a special type of device in Klyff that is able to act as a bridge between external devices connected to different systems and Klyff.
Gateway API provides the ability to exchange data between **multiple devices** and the platform using **single MQTT connection**.
The Gateway also acts as a Klyff device and can leverage existing [MQTT Device API](/docs/{{docsPrefix}}reference/mqtt-api/) to report stats, receive configuration updates and much more.

The API listed below is used by [**Klyff open-source IoT Gateway**](/docs/iot-gateway/what-is-iot-gateway/).
...
In order to inform Klyff that device is connected to the Gateway, one needs to publish following message:
...
Once received, Klyff will lookup or create a device with the name specified.
Also, Klyff will publish messages about new attribute updates and RPC commands for a particular device to this Gateway.

## Device Disconnect API

In order to inform Klyff that device is disconnected from the Gateway, one needs to publish following message:
...
Once received, Klyff will no longer publish updates for this particular device to this Gateway.
...
In order to publish client-side device attributes to Klyff server node, send PUBLISH message to the following topic:
...
In order to request client-side or shared device attributes to Klyff server node, send PUBLISH message to the following topic:
...
In order to publish device telemetry to Klyff server node, send PUBLISH message to the following topic:
