...
We assume you have completed the general [Getting Started](/docs/{{docsPrefix}}getting-started-guides/helloworld/) guide to get familiar with Klyff.
...
This part of documentation covers provisioning of your first LwM2M device in Klyff. We will use [Eclipse Wakaama](https://github.com/eclipse/wakaama#test-client-example) test client to simulate LwM2M device.
...
You may notice that Device Object supports Manufacturer, model, and serial numbers. Let’s configure Klyff to fetch
those data when device connects and store it as Klyff attributes.
Also we want to observe Radio Signal Strength, Link Quality and device location push it as Klyff telemetry.
...
Klyff supports 4 different types of credentials: Pre-Shared Key (PSK), Raw Public Ket (RPK), X.509 Certificates and "No Security" mode.
...
Klyff implements both LwM2M server and bootstrap server that supports plain UDP and DTLS (secure transport over UDP).
As a platform user, you are able to provision and define the mapping between the LwM2M resources and Klyff device [attributes](/docs/{{docsPrefix}}user-guide/attributes/) and [time series data](/docs/{{docsPrefix}}user-guide/telemetry/).
...
#### Read LwM2M resources to Klyff attributes
...
To do this, select the "Attribute" checkbox for the corresponding Resource. You may optionally change the auto-generated key name, defining the attribute name in Klyff.
For example, let's configure the platform to read the LwM2M Resource */3/0/2* (Device Serial Number) and store it as an attribute *serialNumber* in Klyff:

![image](/images/lwm2m/attributes-ce.png)

Klyff will read the attribute value during device registration (LwM2M "Register" operation) or during registration update (LwM2M "Update" operation).

**Note:** 

We may optionally issue the "Observe" operation to make sure we subscribe to the updates of the LwM2M resource.
To do this, select the "Observe" checkbox for the corresponding Resource. Let's configure Klyff to monitor the LwM2M Resource */3/0/15* (Timezone) and store it as the *timezone* attribute:
...
#### Write LwM2M resource via Klyff attributes update

Klyff [Shared Attributes](/docs/{{docsPrefix}}user-guide/attributes/#shared-attributes) are used to deliver the configuration updates to the device.
You may change the shared attribute in multiple ways - via administration UI, dashboard widget, REST API, or rule engine node. 
Once you change the shared attribute, Klyff will search for the mapping between the attribute key and LwM2M resource.
...
To do this, select the "Telemetry" checkbox for the corresponding Resource. You may optionally change the auto-generated key name, defining the telemetry key in Klyff.
...
and to store them as time series data in Klyff:

![image](/images/lwm2m/time-series-ce.png)

#### Execute LwM2M operation using the Klyff RPC command

Klyff supports on-demand LwM2M operations using RPC(Remote Procedure Call) feature. We also use "command" to device instead of RPC for simplicity. 
...
So, the following RPC command need to be sent to Klyff:
...
To use DTLS, the end-user device has to connect to the Klyff server using secured port 5686.
