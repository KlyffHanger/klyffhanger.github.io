...
Klyff rule engine supports basic analysis of incoming telemetry data, for example, threshold crossing.
...
This tutorial will demonstrate how you can:
 ...
 - route telemetry device data from Klyff to Kafka topic using the built-in rule engine capabilities (works for both Klyff CE and PE).
 - aggregate data from multiple devices using a simple Kafka Streams application.
 - push results of the analytics back to Klyff for persistence and visualization using Klyff PE Kafka Integration.
...
Klyff is used to collect, store and visualize anomaly telemetry from these solar modules in each panels.
...
* Klyff PE v2.4.2+ [instance](/docs/user-guide/install/pe/installation-options/)
...
Let's create three devices with type "solar-module". If you are using Klyff PE, you cna put them to new "Solar Modules" group.
...
Klyff subscribed to this topic using Kafka Integration, generate alarms and store anomalies to the database.


#### Download the sample application

Feel free to grab the [code from the Klyff repository](https://github.com/thingsboard/kafka-streams-example) and build the project with maven:
...
Let's configure Klyff to subscribe to the “solar-module-anomalies” topic and create alarms. We will use Kafka Integration that is available since Klyff v2.4.2.
