...
The ChirpStack open-source LoRaWAN Network Server stack provides open-source components for LoRaWAN networks. After integrating ChirpStack with Klyff, you can connect, communicate, process and visualize data from devices in the Klyff IoT platform.
...
Uplink is necessary in order to convert the incoming data from the device into the required format for displaying them in the Klyff.
...
In order for data to be transferred from ChirpStack to Klyff, you need to configure an integration in your ChirpStack application.
...
- Fill in the field with the "HTTP endpoint URL" previously copied from the ChirpStack integration in the Klyff. Then, click "Submit" button.
...
When your device sends an uplink message, a new device will appear in the Klyff user interface.
