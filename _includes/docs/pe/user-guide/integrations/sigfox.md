...
Sigfox Integration allows to stream data from Sigfox Backend to Klyff and converts binary device payloads to the Klyff format.
...
You can сreate an uplink converter in the Data converters page or directly in the integration. Uplink converter is necessary in order to convert the incoming data from the device into the required format for displaying them in Klyff.
...
Now we need to set up a **Sigfox account** so that data from our device is sent to the Klyff platform.
...
Create a callback to connect the Sigfox cloud to your Klyff platform. In the upper right corner, click on the "**New**" button, and select "**Custom callback**".
