...
Remote Integrations allow reliable data streaming from multiple devices in the local area network (LAN) to the Klyff platform in the cloud. 
They are helpful if you have, for example, a local MQTT broker or OPC-UA server with no external IP address and is not reachable from where your Klyff platform is installed. 
The remote integration initiates a connection to these servers, pulls the data, stores it locally, and then streams it to the Klyff instance.
...
Since Klyff is a multi-tenant environment, we can't launch TCP and UDP integrations as part of the core service due to possible port collisions between tenants. 
...
It is possible to execute any Klyff Integration separately from the Klyff instance.

This guide contains step-by-step instructions on how to launch MQTT remote integration that connects to the local MQTT Broker and pushes data to the [Klyff Cloud](https://{{hostName}}/signup).

You can find additional information about the Klyff Platform integrations feature [here](/docs/{{peDocsPrefix}}user-guide/integrations/).

## Klyff configuration

We assume you already have a tenant administrator account on your own [Klyff PE instance](/docs/user-guide/install/pe/installation-options/) or [Klyff Cloud](https://{{hostName}}/signup).

### Create remote integration in Klyff

Let's create remote integration in Klyff that will connect to the local broker using port 1883 and subscribe to all topics.
...
One can install Klyff Integration via Docker, Debian or RPM packages.
...
Remote integration configuration is done via Klyff UI and there is no specific steps.
