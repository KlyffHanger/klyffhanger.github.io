...
After integrating TheThingsIndustries with the Klyff, you can connect, communicate, process and visualize data from devices in the Klyff IoT platform.
...
- **TheThingsIndustries decoder** - data will be decoded before entering the Klyff
- **Klyff converters** - uplink/downlink converters will be used to decode data from binary format into JSON

In this tutorial, we will make an initial transformation into JSON with TTI decoder and then use Klyff converters for correct data processing.
...
Now we can start configuring the Klyff.

##### Klyff Uplink Data Converter
...
##### Klyff Downlink Data Converter
For sending Downlink messages from the Klyff to the device inside TTI, we need to define downlink Converter.
...
Next we will create Integration with TheThingsIndustries inside the Klyff. Open **Integrations** section and add new Integration with type
**TheThingsIndustries**
...
When device sends data, we can check it in the Klyff, to do this:
