...
In this guide, we integrate the TBMQ with the Klyff using MQTT integration. 
We utilize TBMQ client credentials with the type **APPLICATION** to connect Klyff integration as an APPLICATION client.
...
Klyff MQTT Integration acts as an MQTT client. It subscribes to topics and converts the received data into telemetry and attribute updates. 
In case of a downlink message, MQTT integration converts it to the device-suitable format and pushes it to TBMQ. 
Pay attention: TBMQ should be either co-located with the Klyff instance or deployed in the cloud and have a valid DNS name or public static IP address. 
Klyff instance that is running in the cloud can’t connect to the TBMQ deployed in the local network with no internet connection.

### Prerequisites

In this tutorial, we will use:

 - The instance of [Klyff Professional Edition](https://thingsboard.io/docs/user-guide/install/pe/installation-options/) installed **locally**;
 - [TBMQ](https://thingsboard.io/docs/mqtt-broker/install/installation-options/) installed **locally** and accessible by Klyff PE instance;
 - mosquitto_pub MQTT client to send messages.

### TBMQ setup

First, we need to create TBMQ client credentials to use them for connecting Klyff integration to TBMQ.
...
Now you can proceed to the next step - configuration of Klyff integration.

### Klyff setup

In this example, we will use the MQTT integration to connect the Klyff to TBMQ.
...
Now go to the "Sessions" page in the TBMQ UI. Upon successful establishment of the connection between Klyff and TBMQ, we will see a new session and its status - "Connected".
...
After you sent uplink message, go to your integration in Klyff UI and navigate to the "Events" tab. There you'll see the message consumed by the "MQTT Integration".
