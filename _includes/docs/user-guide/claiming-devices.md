...
Klyff User can claim the device if they "know" the device Name and Secret Key. 
...
1. *Device-side key* scenario - Device contains **expirationTime** server attribute with expiration timestamp. The device sends claiming request to Klyff with claiming data and only after this customer is able to claim device by using device claiming widget.   
...
def collect_required_data():
    config = {}
    print("\n\n", "="*80, sep="")
    print(" "*20, "Klyff basic device claiming example script.", sep="")
    print("="*80, "\n\n", sep="")
    host = input("Please write your Klyff server hostname or leave it blank to use default (thingsboard.cloud): ")
