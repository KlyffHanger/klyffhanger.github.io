...
In this system, Klyff serves as the supervisory computer element, gathering data from various PLCs (Programmable Logic Controllers) and RTUs (Remote Terminal Units), 
...
Additionally, Klyff offers robust [alarm](/docs/{{docsPrefix}}user-guide/alarms/){:target="_blank"} and [notification](/docs/{{docsPrefix}}user-guide/notifications/){:target="_blank"} systems, which are essential components of any IoT system.

Starting from release 3.8.0, Klyff introduces support for SCADA-like HMI dashboards, which we will cover in this documentation.

## SCADA dashboard layout

A SCADA dashboard in Klyff provides all the functionalities of a typical platform [Dashboard](/docs/{{docsPrefix}}user-guide/dashboards/){:target="_blank"} with a specialized SCADA [layout](/docs/{{docsPrefix}}user-guide/dashboards/#layouts){:target="_blank"}, 
...
Additionally, you can combine SCADA widgets with over 500 existing Klyff [widgets](/docs/{{docsPrefix}}user-guide/widgets/){:target="_blank"} on the same dashboard.
...
Klyff SCADA symbols are based on [SVG](https://en.wikipedia.org/wiki/SVG){:target="_blank"} (Scalable Vector Graphics) files. 
The use of vector graphics ensures that SCADA symbols scale seamlessly to any screen size. 
Klyff's engineers have extended the SVG format to make these symbols interactive.
