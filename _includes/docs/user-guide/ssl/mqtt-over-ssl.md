...
Klyff provides the ability to run MQTT server over SSL. Both one-way and two-way SSL are supported.

Most of the Klyff environments use the load balancer as a termination point for the SSL connection between the devices and the platform.
...
Nevertheless, it is possible to configure Klyff to two-way SSL for MQTT and avoid SSL termination on the Load Balancer.
...
After completing the setup, start or restart the Klyff server.
