...
Since Klyff 3.2, the Tenant administrator is able to configure common settings for multiple devices using Device Profiles. 
Each Device has one and only profile at a single point in time. 

Experienced Klyff users can notice that the device type has been deprecated in favor of the Device Profile. 
...
To avoid this painful and mundane activity, since Klyff 3.2, you can specify a custom Rule Chain for your devices.
...
The current version of the Klyff platform supports customizable proto schemas for [telemetry upload](/docs/{{docsPrefix}}reference/mqtt-api/#telemetry-upload-api) 
...
Klyff parses the protobuf structures dynamically, that is why, it does not support some protobuf features like OneOf, extensions and maps, yet.
...
The current version of the Klyff platform supports customizable proto schemas for [telemetry upload](/docs/{{docsPrefix}}reference/coap-api/#telemetry-upload-api) 
...
The current version of the Klyff platform supports integration with next Efento NB-IoT sensors: 
...
Since Klyff 3.2, we have introduced Alarm Rules to simplify the process of configuring the most popular alarm types.
...
Assuming you have configured alarm rules you may also want to receive a notification when Klyff creates or updates the alarm.
...
Device provisioning allows a device to automatically register in Klyff either during or after manufacturing. 
