...
Since Klyff 3.3, Klyff allows you to upload and distribute over-the-air(OTA) updates to devices. 
...
Klyff will notify devices about the available update and provide a protocol-specific API to download the firmware. 
...
## Provision OTA package to Klyff repository
...
There is a very minor difference between FOTA and SOTA implementation inside Klyff core. Many use-cases and applications will need to use FOTA only. 
...
Klyff tracks the progress of the update and persists it to the device attributes.
...
Klyff queues the update notifications to avoid peak loads. The queue is processed with the constant pace. 
...
Under the hood, Klyff converts notification to the update of the following [shared attributes](/docs/{{docsPrefix}}user-guide/attributes/#shared-attributes):
...
Once the firmware/software is updated, Klyff expect the device to send the following telemetry:
...
If the firmware/software update failed, Klyff expect the device to send the following telemetry:
...
Klyff provides the summary of the firmware/software update to monitor and track the firmware/software update status of your device, such as which devices are updating right now, any boot issues, and which ones have already been updated.

### Firmware update monitoring dashboard

The dashboard is created automatically for each new tenant that you add to Klyff. 
