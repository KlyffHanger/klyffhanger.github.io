...
Klyff provides a rich set of features related to time-series data:
...
Klyff internally treats time-series data as timestamped key-value pairs. We call single timestamped key-value pair a **data point**. 
...
You may notice that the JSON listed above does not have a timestamp information. In such case, Klyff uses current server timestamp. 
...
We assume you have already pushed time-series data to Klyff. Now you may use it in your dashboards. 
...
Klyff Cloud stores time-series data in the Cassandra database with replication factor of 3. 
The on-prem installation of Klyff support storage of time-series data in SQL (PostgreSQL) or NoSQL (Cassandra or Timescale) databases.

{% else %}

System administrator is able to configure Klyff to store time-series data in SQL (PostgreSQL) or NoSQL (Cassandra or Timescale) databases.
...
Klyff Cloud stores data with configurable time-to-live (TTL) parameter. 
...
The device that sends message with time-series data to Klyff will receive confirmation 
...
Klyff provides following REST API to fetch entity data:
...
Klyff will use *startTs*, *endTs*, and *interval* to identify aggregation partitions or sub-queries and execute asynchronous queries to DB that leverage built-in aggregation functions.
