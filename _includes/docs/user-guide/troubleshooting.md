...
It is possible that the data inside the cache somehow got corrupted. Regardless of the reason, it is always safe to clear cache, Klyff will just refill it at the runtime.
...
Regardless of the deployment type, Klyff logs are stored on the same server/container as Klyff Server/Node itself in the following directory:
...
Klyff provides the ability to enable/disable logging for certain parts of the system depending on what information do you need for troubleshooting.

You can do this by modifying <b>logback.xml</b> file. As logs itself, it is stored on the same server/container as Klyff Server/Node in the following directory:
...
If you are running Klyff as microservices with separate services for MQTT and COAP transport, you also need to set environment variables `WEB_APPLICATION_ENABLE` to value `true`, 
...
Here's the list of metrics Klyff pushes to Prometheus.
...
Also, you can view Grafana dashboards after deploying Klyffs docker-compose cluster configuration (for more information please follow [this guide](/docs/user-guide/install/{{docsPrefix}}cluster/docker-compose-setup/)).
...
Sometimes after configuring OAuth you can not see the button for logging in with OAuth provider. This happens when “Domain name” and “Redirect URI Template” contain faulty values, they need to be the same you use for accessing your Klyff web page.
...
> Go to your Klyff as a System Administrator. Check the General
...
<section id="talkToUs">
    <div id="gettingHelp">
        ...
        <a href="https://stackoverflow.com/questions/tagged/thingsboard">
            ...
            <p>The Klyff team will also monitor posts tagged thingsboard. If there aren’t any existing questions that help, please ask a new one!</p>
        </a>
    </div>
</section>

If your problem isn't answered by any of the guides above, feel free to contact Klyff team.
