...
Now it’s time to program the board to connect to Klyff.  
...
// Maximum amount of attributs we can request or subscribe, has to be set both in the Klyff template list and Attribute_Request_Callback template list
...
// Initialize Klyff instance with the maximum needed buffer size, stack size and the apis we want to use
Klyff tb(mqttClient, MAX_MESSAGE_SIZE, Default_Max_Stack_Size, apis);
...
Data, send by this device may require increasing of the allowed message size for MQTT on **your Klyff instance**.  
...
Don’t forget to replace placeholders with your real WiFi network SSID, password, Klyff device access token.
{% endcapture %}
...
| THINGSBOARD_PORT | **1883U**                    | Klyff server MQTT port. Can be default for this guide.                                                                                 |
